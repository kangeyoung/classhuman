...
public class ProductM {
    ...
    ProductM() throws ClassNotFoundException, SQLException {
        ...
        while (true) {
            System.out.println("1. 등록 2. 조회 3. 수정 4. 찜하기 5. 종료");
    ...
    private void love() throws SQLException {
        Scanner sc = new Scanner(System.in);
        ArrayList<ProductDTO> productList = productDAO.select(1, null);
        if (!productList.isEmpty()) {
            for (ProductDTO product : productList) {
                product.prt();
            }
        }
        System.out.println("찜할 아이디 선택");
        String id = sc.nextLine();
        productDAO.updatelove(id);
    }
