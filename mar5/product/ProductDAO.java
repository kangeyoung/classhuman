...
public class ProductDAO {
    ...
    public void add(ProductDTO productDTO) throws SQLException {
        String sql = "insert into product values(?,?,?,?,?,?,?,default,default)";
        Connection conn = DriverManager.getConnection(url, id, pass);
    ...
    public ArrayList<ProductDTO> select(int i, String text) throws SQLException {
        ...
        switch (i) {
            case 1:
                sql = "select id, name, quantity, price, typename, manager, description, indate, love " +
                        "from product p, typeP t where p.type = t.type order by p.id";
                break;
            case 2:
                sql = "select id, name, quantity, price, typename, manager, description, indate, love " +
        ...
        while (rs.next()) {
            ...
            productDTO.setLove(rs.getInt("love"));
    ...
    public void updatelove(String id) throws SQLException {
        String sql = "update product set love = 1 where id =?";
        Connection conn = DriverManager.getConnection(url, this.id, pass);
        PreparedStatement pstmt = conn.prepareStatement(sql);
