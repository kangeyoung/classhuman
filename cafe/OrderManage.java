...
public class OrderManage {
    ...
    private void reviewCus() {
        ...
        for (int i = 0; i < 20; i++) {
            if (mn.reviewList[i] != null && cName.equals(mn.reviewList[i].cName)) {
                mn.reviewList[i].prtReview();
                break;
            }
        }
    }
    ...
    private void reviewOrder(int i) {
        for (int j = 0; j < 5; j++) {
            ...
            if (menuName != null) {
                ...
                for (int s = 0; s < mn.reviewList.length; s++) {
                    if (mn.reviewList[s] == null) {
                        mn.reviewList[s] = review;
                    }
                }
    ...
    private void addOrder() {
        ...
        for (int i = 0; i < orderList.length; i++) {
            if (orderList[i] == null){
                orderList[i] = order;
                orderList[i].OrderMenu(mn.menuList);
                break;
